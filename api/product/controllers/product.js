...
module.exports = {
  ...
  async findAnd(ctx) {
    ...
    if (ctx.state.user.role.name === "Doctor") {
      ...
      let specialties = await strapi.services['specialties-has-user'].find({ 'users_id': userId })
      specialties.map(item => {
        userSpecialties.push(item.specialty_id.specialty)
      })
      // extracting availability_hours from model
      let availabilityHours = await strapi.services['availability-hour'].find({ 'users_id': userId })
      ...
      let medicalAppointment = await strapi.services['medical-appointment'].find({ 'professional_id': userId })
  ...
  async search(ctx) {
    ...
    let query = "SELECT concat('Dr ',users.first_name, ', ', users.surname) as 'professionalFullName',\n" +
    ...
    result[0].map(item => {
      item.schedules = []
      if (before != undefined && item.professionalFullName == before.professionalFullName) {
        array[array.length - 1].schedules.push(JSON.parse(item.availabilityHour))
      }
